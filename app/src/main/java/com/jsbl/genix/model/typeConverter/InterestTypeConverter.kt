...
class InterestTypeConverter {

    var gson = Gson()

    @TypeConverter
    fun stringToInterest(data: String?): List<Interest> {
        if (data == null) {
        ...
        try {
            val listType: Type =
                object : TypeToken<List<Interest?>?>() {}.getType()
    ...
    @TypeConverter
    fun InterestListToString(someObjects: List<Interest?>?): String? {
        if (someObjects == null) {
            return null
        }
